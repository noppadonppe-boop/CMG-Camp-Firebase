...
// ─── NOTE FOR DEVELOPERS ────────────────────────────────────────────────────
// This component writes to Firebase Firestore and Firebase Storage.
// Before use, populate .env.local with your Firebase credentials.
// See .env.local.example and lib/firebase.ts for setup instructions.
// ────────────────────────────────────────────────────────────────────────────
...
/* ─── Hardcoded inspector (replace with Firebase Auth currentUser) ── */
...
const RESULT_CONFIG: Record<
  ...
  { label: string; labelEn: string; icon: React.ElementType; active: string }
> = {
  pass: {
    label: "ผ่าน",
    labelEn: "Pass",
  ...
  na: {
    ...
    labelEn: "N/A",
    icon: MinusCircle,
    active: "bg-gray-400 text-white border-gray-400 shadow-sm",
  },
};
...
function ResultButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex items-center gap-1.5 rounded-xl border-2 px-3 py-2 text-sm font-semibold transition active:scale-95
        ${selected ? cfg.active : "border-gray-200 bg-white text-gray-500 hover:border-gray-300 hover:bg-gray-50"}`}
    >
      ...
      {preview ? (
        <div className="relative inline-block">
          <img
            src={preview}
            alt="หลักฐานภาพ"
            className="h-20 w-28 rounded-xl border border-gray-200 object-cover shadow-sm"
          />
          <button
            type="button"
            onClick={onClear}
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl border-2 border-dashed border-gray-300 px-4 py-2.5 text-sm text-gray-400 transition hover:border-blue-400 hover:text-blue-500"
        >
...
function ChecklistItemRow({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`mt-0.5 h-2.5 w-2.5 shrink-0 rounded-full ${
            state.status === "pass" ? "bg-emerald-400" :
            state.status === "fail" ? "bg-red-400" :
            state.status === "na"   ? "bg-gray-300" : "bg-gray-200"
          }`}
        />
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {(["pass", "fail", "na"] as const).map((v) => (
          <ResultButton
            ...
            onClick={() => onChange({ status: state.status === v ? null : v })}
          />
        ))}

        {/* Note toggle — shown always for fail, optional for others */}
        <button
          type="button"
          onClick={() => setShowNote((s) => !s)}
          className={`ml-auto flex items-center gap-1 rounded-xl border px-3 py-2 text-xs transition ${
            state.note
      ...
      {/* Expanded note + photo area */}
      {(showNote || hasIssue) && (
        <div className="mt-3 space-y-2">
          <textarea
            value={state.note}
            onChange={(e) => onChange({ note: e.target.value })}
            placeholder={hasIssue ? "ระบุรายละเอียดปัญหา (จำเป็น)..." : "หมายเหตุเพิ่มเติม..."}
            rows={2}
          ...
          <PhotoUpload
            ...
            onChange={(file, preview) => onChange({ photoFile: file, photoPreview: preview })}
            onClear={() => onChange({ photoFile: null, photoPreview: null })}
          />
        </div>
      )}
    </div>
      ...
      >
        ...
        <span
          className={`h-3 w-3 shrink-0 rounded-full ${
            ...
            allPass       ? "bg-emerald-400" :
            answered > 0  ? "bg-amber-400" : "bg-gray-300"
          }`}
        />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <div className={`mt-4 rounded-xl border p-4 ${overallCfg.bg} ${overallCfg.border}`}>
          <p className={`text-center text-2xl font-bold ${overallCfg.color}`}>
            ผล: {overallCfg.label}
          </p>
        ...
        {failCount > 0 && (
          <div className="mt-3 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
            ...
            ระบบจะสร้างค่าปรับโดยอัตโนมัติสำหรับห้องที่ไม่ผ่าน
          </div>
        )}
        ...
        <div className="mt-5 flex gap-3">
          <button
            ...
            className="flex-1 rounded-xl border border-gray-200 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 transition disabled:opacity-50"
          >
            ยกเลิก
          </button>
          <button
            onClick={onConfirm}
            disabled={submittingModal}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 active:scale-95 transition disabled:opacity-60"
          >
            {submittingModal ? <Loader2 className="h-4 w-4 animate-spin" /> : <CloudUpload className="h-4 w-4" />}
            ยืนยันส่งผล
          </button>
        </div>
      </div>
...
function SuccessScreen({
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center text-center px-4">
      <div className={`mb-4 flex h-20 w-20 items-center justify-center rounded-full border-2 text-4xl ${cfg.bg} ${cfg.border}`}>
        {cfg.icon}
      </div>
      <h2 className={`text-2xl font-bold ${cfg.color}`}>ผล: {cfg.label}</h2>
      <p className="mt-1 text-gray-500">{cfg.sub}</p>
      <div className="mt-2 rounded-xl bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-700">
        ห้อง {roomNumber}
      </div>
      {failCount > 0 && (
        <p className="mt-3 text-sm text-red-500">
          มี {failCount} รายการที่ไม่ผ่าน — ค่าปรับถูกสร้างในระบบบิลแล้ว
        </p>
      )}
      <button
        onClick={onReset}
        className="mt-8 flex items-center gap-2 rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 active:scale-95 transition"
      >
...
export default function InspectFormPage() {
  ...
  const failsWithoutNote = allItems.filter(
    (i) => states[i.id]?.status === "fail" && !states[i.id]?.note.trim()
  );
  const canSubmit =
    !!selectedRoomId &&
    answered === total &&
  ...
  const filteredRooms = roomsWithZone.filter(
    (r) =>
    ...
    <div className="pb-32">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          กรอกผลการตรวจสอบความสะอาดและสุขอนามัยในแต่ละห้องพัก
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
          {filteredRooms.map((room) => (
            <button
              key={room.id}
              type="button"
              onClick={() => { handleReset(); setSelectedRoomId(room.id); }}
              className={`rounded-xl border-2 px-3 py-3 text-left transition active:scale-95 ${
                selectedRoomId === room.id
                  ? "border-blue-500 bg-blue-50 shadow-sm"
                  : "border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50"
              }`}
            >
              <p className={`text-sm font-bold ${selectedRoomId === room.id ? "text-blue-700" : "text-gray-800"}`}>
                {room.number}
              </p>
      ...
      {selectedRoomId && (
        <div className="mb-5">
          ...
          <div className="h-2.5 w-full rounded-full bg-gray-200">
            <div
              className={`h-2.5 rounded-full transition-all duration-300 ${progressColor(pct)}`}
              style={{ width: `${pct}%` }}
            />
          </div>
          {/* Mini stat row */}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border-2 border-gray-200 bg-white p-5 shadow-sm">
            ...
            <textarea
              ...
              placeholder="สรุปสภาพทั่วไปของห้องพัก หรือข้อสังเกตอื่นๆ..."
              rows={3}
              className="w-full resize-none rounded-xl border border-gray-300 px-3 py-2.5 text-sm outline-none hover:border-gray-400 focus:ring-2 focus:ring-blue-400"
            />
          </div>
          ...
          {answered < total && (
            <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
              ...
              <p className="text-sm text-amber-700">
                ยังมี <strong>{total - answered} รายการ</strong> ที่ยังไม่ได้ประเมิน
              </p>
            </div>
          )}
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-gray-200 bg-white/95 px-4 py-3 backdrop-blur-sm sm:left-60">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-3">
          <div className="text-sm text-gray-500 hidden sm:block">
            {selectedRoom ? (
              <span>
                ห้อง <strong className="text-gray-800">{selectedRoom.number}</strong>
                {" · "}ตอบแล้ว{" "}
                <strong className="text-blue-600">{answered}/{total}</strong>
              </span>
            ) : (
              <span className="text-gray-400">ยังไม่ได้เลือกห้อง</span>
            )}
          </div>
